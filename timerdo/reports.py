...
def projects(
    ...
    project: Optional[List[str]] = typer.Option(
        ...
        False, '--done', '-d', help='Flag to see done tasks.'
    ),
):
    ...
    with Session(engine) as session:
        tasks = session.exec(
            select(ToDo)
            .where(project, done, ToDo.date_init >= init)
            .order_by(ToDo.project)
        ).all()
        project_init = {}
        for task in tasks:
            if (
                task.project in project_init.keys()
                and project_init[task.project] < task.date_init
            ):
                pass
            else:
...
def tags(
    ...
    tag: Optional[List[str]] = typer.Option(
        None, '--tag', '-t', help='Flag more than one.'
    ),
    done: bool = typer.Option(
        False, '--done', '-d', help='Flag to see done tasks.'
    ),
    ...
    with Session(engine) as session:
        tasks = session.exec(
            select(ToDo.tag)
            .where(tag, done, ToDo.date_init >= init)
            .order_by(ToDo.tag)
            .distinct()
        ).all()
        print(tasks)
...
def task(id: int):
    ...
    try:
        ...
        table_task = [
            ['Start', 'End', 'Tag', 'Remarks', 'Reminder'],
            [
                task['date_init'],
                task['date_end'],
                task['tag'],
...
""",
        ...
        for timer in timers:
            ...
            values = [
                timer['id'],
                timer['start'].strftime('%Y-%m-%d %H:%M'),
